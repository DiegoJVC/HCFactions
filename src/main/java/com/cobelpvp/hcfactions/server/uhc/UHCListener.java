...
public class UHCListener implements Listener {
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        if (!deathTime.containsKey(damaged.getUniqueId()) || (System.currentTimeMillis() - deathTime.get(damaged.getUniqueId()) > 200L)) {
                            damager.sendMessage(damaged.getDisplayName() + ChatColor.YELLOW + " is now at " + ChatColor.RED + formatHearts(damaged) + ChatColor.YELLOW + ".");
                        }
                    }
    ...
    private static String formatHearts(Player player) {
        return HEARTS_FORMAT.format(getHearts(player)) + (HEART_CHAR);
    }
