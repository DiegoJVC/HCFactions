...
public class TeamSubclaimCreateCommand {
    ...
    @Command(names = {"f subclaim create", "faction subclaim create", "fac subclaim create", "f sub create", "faction sub create", "fac sub create"}, permission = "")
    public static void teamSubclaimCreate(Player sender, @Param(name = "subclaim") String subclaim) {
        ...
        if (!faction.isOwner(sender.getUniqueId()) && !faction.isCoLeader(sender.getUniqueId()) && !faction.isCaptain(sender.getUniqueId())) {
            sender.sendMessage(ChatColor.RED + "Only faction captains can create subclaims!");
            return;
        }

        if (faction.getSubclaim(subclaim) != null) {
            sender.sendMessage(ChatColor.RED + "Your faction already has a subclaim with that name!");
            return;
        }
        ...
        for (Location loc : new CuboidRegion("Subclaim", selection.getLoc1(), selection.getLoc2())) {
            if (LandBoard.getInstance().getTeam(loc) != faction) {
                sender.sendMessage(ChatColor.RED + "This subclaim would conflict with the claims of faction §e" + LandBoard.getInstance().getTeam(loc).getName() + "§c!");
                return;
            }

            Subclaim subclaimAtLoc = faction.getSubclaim(loc);
