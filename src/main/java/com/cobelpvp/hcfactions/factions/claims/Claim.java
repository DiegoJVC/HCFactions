...
public class Claim implements Iterable<Coordinate> {
    ...
    public static int getPrice(Claim claim, Faction faction, boolean buying) {
    ...
    public BasicDBObject json() {
        ...
        World world = HCFactions.getInstance().getServer().getWorld(getWorld());
    ...
    public Location getMinimumPoint() {
        return (new Location(HCFactions.getInstance().getServer().getWorld(world), Math.min(x1, x2), Math.min(y1, y2), Math.min(z1, z2)));
    }

    public Location getMaximumPoint() {
        return (new Location(HCFactions.getInstance().getServer().getWorld(world), Math.max(x1, x2), Math.max(y1, y2), Math.max(z1, z2)));
    }
    ...
    public Location[] getCornerLocations() {
        World world = HCFactions.getInstance().getServer().getWorld(this.world);
