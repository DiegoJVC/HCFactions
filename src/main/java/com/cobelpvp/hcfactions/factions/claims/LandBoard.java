...
public class LandBoard implements Listener {
    ...
    private final Map<String, Multimap<CoordinateSet, Map.Entry<Claim, Faction>>> buckets = new ConcurrentHashMap<>();
    @Getter
    @Setter
    private boolean claimsEnabled = true;

    public LandBoard() {
        for (World world : HCFactions.getInstance().getServer().getWorlds()) {
    ...
    public void loadFromTeams() {
        for (Faction faction : HCFactions.getInstance().getFactionHandler().getTeams()) {
            for (Claim claim : faction.getClaims()) {
                setTeamAt(claim, faction);
            }
        }
    }

    public Set<Map.Entry<Claim, Faction>> getRegionData(Location center, int xDistance, int yDistance, int zDistance) {
    ...
    public Set<Map.Entry<Claim, Faction>> getRegionData(Location min, Location max) {
        if (!claimsEnabled) {
            return (new HashSet<>());
        }

        Set<Map.Entry<Claim, Faction>> regions = new HashSet<>();
        ...
        for (int x = min.getBlockX(); x < max.getBlockX() + step; x += step) {
            for (int z = min.getBlockZ(); z < max.getBlockZ() + step; z += step) {
                ...
                for (Map.Entry<Claim, Faction> regionEntry : buckets.get(min.getWorld().getName()).get(coordinateSet)) {
    ...
    public Map.Entry<Claim, Faction> getRegionData(Location location) {
        if (!claimsEnabled) {
            return (null);
        }

        for (Map.Entry<Claim, Faction> data : buckets.get(location.getWorld().getName()).get(new CoordinateSet(location.getBlockX(), location.getBlockZ()))) {
    ...
    public Claim getClaim(Location location) {
        Map.Entry<Claim, Faction> regionData = getRegionData(location);
        return (regionData == null ? null : regionData.getKey());
    }

    public Faction getTeam(Location location) {
        Map.Entry<Claim, Faction> regionData = getRegionData(location);
        return regionData == null ? null : regionData.getValue();
    }

    public void setTeamAt(Claim claim, Faction faction) {

        Map.Entry<Claim, Faction> regionData = new AbstractMap.SimpleEntry<>(claim, faction);
        int step = 1 << CoordinateSet.BITS;
        ...
        for (int x = regionData.getKey().getX1(); x < regionData.getKey().getX2() + step; x += step) {
            for (int z = regionData.getKey().getZ1(); z < regionData.getKey().getZ2() + step; z += step) {
                Multimap<CoordinateSet, Map.Entry<Claim, Faction>> worldMap = buckets.get(regionData.getKey().getWorld());
                ...
                if (regionData.getValue() == null) {
                    CoordinateSet coordinateSet = new CoordinateSet(x, z);
                    Iterator<Map.Entry<Claim, Faction>> claimIterator = worldMap.get(coordinateSet).iterator();

                    while (claimIterator.hasNext()) {
                        Map.Entry<Claim, Faction> entry = claimIterator.next();
