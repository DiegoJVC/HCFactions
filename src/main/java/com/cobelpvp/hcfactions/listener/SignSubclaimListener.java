...
public class SignSubclaimListener implements Listener {
    ...
    public void onSignUpdate(SignChangeEvent event) {
        ...
        if (!playerFaction.ownsLocation(sign.getLocation())) {
            event.getBlock().breakNaturally();
    ...
    public void onBlockBreakChest(BlockBreakEvent event) {
        if (!(event.getBlock().getState() instanceof Chest) || HCFactions.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getBlock().getLocation()) || HCFactions.getInstance().getServerHandler().isAdminOverride(event.getPlayer())) {
            return;
        }

        Faction owningFaction = LandBoard.getInstance().getTeam(event.getBlock().getLocation());
        UUID uuid = event.getPlayer().getUniqueId();

        for (Sign sign : subclaimSigns(event.getBlock())) {
            if (!(owningFaction.isOwner(uuid) || owningFaction.isCoLeader(uuid) || owningFaction.isCaptain(uuid))) {
    ...
    public void onBlockBreakSign(BlockBreakEvent event) {
        if (!(event.getBlock().getState() instanceof Sign) || HCFactions.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getBlock().getLocation()) || HCFactions.getInstance().getServerHandler().isAdminOverride(event.getPlayer())) {
            return;
        }
        ...
        if (sign.getLine(0).equals(SUBCLAIM_IDENTIFIER)) {
            boolean canAccess = owningFaction.isOwner(uuid) || owningFaction.isCoLeader(uuid) || owningFaction.isCaptain(uuid);
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (event.getAction() != Action.RIGHT_CLICK_BLOCK || !(event.getClickedBlock().getState() instanceof Chest) || HCFactions.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getClickedBlock().getLocation()) || HCFactions.getInstance().getServerHandler().isAdminOverride(event.getPlayer())) {
            return;
        }

        // Will never be null, we check isUnclaimedOrRaidable above.
        Faction owningFaction = LandBoard.getInstance().getTeam(event.getClickedBlock().getLocation());
        UUID uuid = event.getPlayer().getUniqueId();
