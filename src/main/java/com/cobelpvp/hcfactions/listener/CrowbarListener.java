    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (!HCFactions.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getClickedBlock().getLocation()) && !HCFactions.getInstance().getServerHandler().isAdminOverride(event.getPlayer())) {
            ...
            if (faction != null && !faction.isMember(event.getPlayer().getUniqueId())) {
                event.getPlayer().sendMessage(ChatColor.YELLOW + "You cannot crowbar in " + ChatColor.RED + faction.getName(event.getPlayer()) + ChatColor.YELLOW + "'s territory!");
                return;
            }
        }
        ...
        if (event.getClickedBlock().getType() == Material.ENDER_PORTAL_FRAME) {
            ...
            HCFactions.getInstance().getServer().getPluginManager().callEvent(crowbarSpawnerBreakEvent);
