...
public final class ServerFakeFreezeTask extends BukkitRunnable {
    ...
    private int playerCount() {
        return HCFactions.getInstance().getServer().getOnlinePlayers().size();
    }
    ...
    private void freeze() {
        ...
        for (Player player : HCFactions.getInstance().getServer().getOnlinePlayers()) {
            if (player.isOp()) {
                player.sendMessage(ChatColor.DARK_PURPLE.toString() + ChatColor.BOLD + "HCFactions autofreeze would have frozen the server.");
            }
        }

        HCFactions.getInstance().getLogger().info("HCFactions autofreeze would have frozen the server.");
    }
    ...
    private void unfreeze() {
        ...
        for (Player player : HCFactions.getInstance().getServer().getOnlinePlayers()) {
            if (player.isOp()) {
                player.sendMessage(ChatColor.DARK_PURPLE.toString() + ChatColor.BOLD + "HCFactions autofreeze would have unfrozen the server.");
            }
        }

        HCFactions.getInstance().getLogger().info("HCFactions autofreeze would have unfrozen the server.");
    }
