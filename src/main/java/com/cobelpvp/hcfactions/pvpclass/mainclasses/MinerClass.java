...
public class MinerClass extends PvPClass implements Listener {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                for (String key : new HashMap<>(noDamage).keySet()) {
                    ...
                    Player player = HCFactions.getInstance().getServer().getPlayerExact(key);
                    ...
                    if (left == 0) {
                        if (player.getLocation().getY() <= Y_HEIGHT) {
                            ...
                            player.sendMessage(ChatColor.BLUE + "Miner Invisibility" + ChatColor.GREEN + " will be activated in 10 seconds!");
                        }
                    } else {
                        ...
                        if (secs == 0) {
                            if (player.getLocation().getY() <= Y_HEIGHT) {
                                if (!(player.hasPotionEffect(PotionEffectType.INVISIBILITY))) {
                                    player.sendMessage(ChatColor.BLUE + "Miner Invisibility" + ChatColor.GREEN + " has been enabled!");
    ...
    public void apply(Player player) {
        ...
        if (!HCFactions.getInstance().getServerHandler().isHardcore()) player.addPotionEffect(new PotionEffect(PotionEffectType.FIRE_RESISTANCE, Integer.MAX_VALUE, 0), true);
    }
    ...
    public void tick(Player player) {
        ...
        int diamonds = HCFactions.getInstance().getDiamondMinedMap().getMined( player.getUniqueId() );
        int level = 1;
        if (diamonds > 125) {
            level = 3;
        } else if (diamonds > 50) {
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        if (invis.containsKey(player.getName()) && invis.get(player.getName()) != 0){
            ...
            player.sendMessage(ChatColor.BLUE + "Miner Invisibility" + ChatColor.RED + " has been temporarily removed!");
        }
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (event.getTo().getBlockY() <= Y_HEIGHT) { // Going below 20
            ...
            if (invis.containsKey(player.getName())) {
                ...
                invis.remove(player.getName());
                player.removePotionEffect(PotionEffectType.INVISIBILITY);
                player.sendMessage(ChatColor.BLUE + "Miner Invisibility" + ChatColor.RED + " has been removed!");
            }
        }
    }
